...
export default class BlameGutter {
  ...
  bindCopyShaToCliboard(){
    this.gutter().element.addEventListener('click', function(event){
      ...
      const targetParentElement = target && target.parentElement;
      if (target && targetParentElement &&
        targetParentElement.tagName == 'A' &&
        targetParentElement.classList.contains('hash-no-view-url')
      ){
        let elementWithHash = targetParentElement && targetParentElement.querySelector('.hash');
        if (elementWithHash) {
          atom.clipboard.write(elementWithHash.innerText);
          atom.notifications.addSuccess('SHA ' + elementWithHash.innerText + ' copied to clipboard');
        }
      }
    }, false);
  }
